//! Metal GPU Bootstrap End-to-End Test (NATIVE GPU RESCALING - EXPERIMENTAL)
//!
//! Tests the complete V3 bootstrap pipeline on Metal GPU using GPU-native RNS rescaling.
//! This is an experimental version that replaces CPU BigInt rescaling with GPU-native
//! RNS formula: r'ᵢ = (rᵢ - r_top) × q_top^{-1} mod qᵢ
//!
//! **Current Status:**
//! - Produces ~385k errors (vs ~3.6e-3 with hybrid CPU rescaling)
//! - RNS approximation doesn't handle CKKS centered rounding correctly
//! - Use for performance benchmarking and rescaling algorithm experimentation
//!
//! **Run:**
//! ```bash
...
fn main() -> Result<(), String> {
    ...
    use ga_engine::clifford_fhe_v2::backends::gpu_metal::bootstrap::{coeff_to_slot_gpu_native, slot_to_coeff_gpu_native};
    use std::time::Instant;

    println!("╔═══════════════════════════════════════════════════════════════════════════════╗");
    println!("║   Metal GPU V3 Bootstrap - NATIVE GPU RESCALING (EXPERIMENTAL)               ║");
    println!("║   Expected: ~385k errors (vs ~3.6e-3 with hybrid CPU rescaling)              ║");
    println!("╚═══════════════════════════════════════════════════════════════════════════════╝\n");
    ...
    let ct_slots = coeff_to_slot_gpu_native(&ct_original, &metal_rot_keys, &metal_ctx, &params)?;
    ...
    let ct_coeffs = slot_to_coeff_gpu_native(&ct_slots, &metal_rot_keys, &metal_ctx, &params)?;
    ...
    // Trim ciphertext to active primes before decryption
    // After rescaling, ct_coeffs has level=1 (2 active primes) but arrays are padded to 20 primes
    let ct_trimmed = ct_coeffs.trim_to_active_primes();
    let start_decrypt = Instant::now();
    let pt_result = metal_ctx.decrypt(&ct_trimmed, &sk)?;
