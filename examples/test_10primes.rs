...
fn main() {
    let params = CliffordFHEParams::new_rns_mult(); // 10 primes
    println!("Testing with {} primes (production config)", params.moduli.len());
    ...
    println!("Error: {} ({:.6}%)", (decoded - value).abs(), (decoded - value).abs() / value * 100.0);

    if (decoded - value).abs() < 0.01 {
        println!("✅ PASS");
    } else {
        println!("❌ FAIL");
    }
