...
fn main() {
    // Create params with only 2 primes
    let mut params = CliffordFHEParams::new_rns_mult();
    params.moduli = params.moduli[..2].to_vec(); // Keep only first 2 primes
    ...
    println!("First 2 primes: {}, {}", params.moduli[0], params.moduli[1]);
